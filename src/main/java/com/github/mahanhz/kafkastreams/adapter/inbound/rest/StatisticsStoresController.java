...
@RestController
@RequestMapping("/statistics/stores")
public class StatisticsStoresController {

    private final StoreService storeService;

    public StatisticsStoresController(final StoreService storeService) {
        this.storeService = storeService;
    }

    @GetMapping("local/{year}/{carMake}")
    public CarSaleStatistic aggregation(@PathVariable final String year,
                                        @PathVariable final String carMake) {
        return storeService.carSaleStatisticInLocalStore(year, carMake);
    }
}
